  ...
  describe ".render" do
    ...
    context "with locals substitution" do
      it "makes locals available as variables in the template" do
        # Create a temp test via the generate_suggestions user prompt which uses all locals
        result = described_class.render(
          step: "generate_suggestions",
          kind: :user,
          locals: { handle: "myhandle", followers: 9999, profile_metrics: {}, insights: {}, target_count: 3, mix_label: "3 reels" }
        )
    ...
    context "rendered output is stripped" do
      it "removes leading and trailing whitespace" do
        result = described_class.render(step: "analyze_reels", kind: :system, locals: {})
